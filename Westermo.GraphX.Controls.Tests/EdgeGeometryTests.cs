...
namespace Westermo.GraphX.Controls.Tests;
...
public class EdgeGeometryTests
{
        ...
        bool selfLoop = false, int parallelEdges = 0)
    {
        ...
        // generate edges now
        area.GenerateAllEdges(updateLayout: true);
    ...
    public async Task ParallelEdges_AssignDifferentOffsets()
    {
        ...
        var pairs = list.Select(e => (e.SourceEndpoint, e.TargetEndpoint)).ToList();
...
public class EllipseDescriptor(EllipseGeometry geometry)
{
    ...
    public Rect Rect { get; } = geometry.Bounds;
}
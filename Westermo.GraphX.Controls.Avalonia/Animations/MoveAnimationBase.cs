    ...
    public abstract class MoveAnimationBase
    {
        ...
        public void AddVertexData(IGraphControl control, Measure.Point coord)
        {
            if (double.IsNaN(coord.X) || double.IsNaN(coord.Y))
                throw new GX_InvalidDataException(
                    "AddVertexData() -> NaN coordinated has been supplied! Correct coordinates was expected.");
            if (!VertexStorage.TryAdd(control, coord))
                throw new GX_GeneralException(
                    "AddVertexData() -> Same control can't be loaded in animation list twice!");
        }
        ...
        public void AddEdgeData(IGraphControl control)
        {
            if (!EdgeStorage.Contains(control))
                ...
                throw new GX_GeneralException("AddEdgeData() -> Same control can't be loaded in animation list twice!");
        }

...
namespace ShowcaseApp.WPF.External.WindowGlow
{
    ...
    {
        private static readonly DependencyProperty GlowInfoProperty =
        ...
        private static readonly Action<Window> assignGlows = window =>
        {
            ...
            foreach (var glow in glowInfo.glows)
            {
                glow.Owner = window;
                glow.OwnerChanged();
            }
        ...
        private static void Assign(Window window)
        {
            ArgumentNullException.ThrowIfNull(window);
            if (GetGlowInfo(window) != null)
                return;
            if (!window.IsLoaded)
                window.SourceInitialized += delegate { assignGlows(window); };
            else
            {
                assignGlows(window);
            }
        ...
        private static void Unassign(Window window)
        {
            var info = GetGlowInfo(window);
            if (info == null)
                return;
            foreach (var glow in info.glows)
            {
                try
                {
                    glow.Close();

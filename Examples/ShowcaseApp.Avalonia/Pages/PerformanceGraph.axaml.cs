...
public partial class PerformanceGraph : UserControl
{
    ...
    private void ApplySettings()
    {
        ...
        {
            edge.EnableGeometryCaching = chkEnableCaching.IsChecked == true;
        }
        
        // Edge Throttling
        if (int.TryParse(txtThrottleMs.Text, out var throttle))
        {
            foreach (var edge in graphArea.EdgesList.Values)
            {
                if (edge is EdgeControl ec)
                    ec.UpdateThrottleMs = throttle;
            }
        }
    }
    ...
    private void OnCachingChanged(object? sender, RoutedEventArgs e)
    {
        ...
        foreach (var edge in graphArea.EdgesList.Values)
        {
            edge.EnableGeometryCaching = enabled;
        }
    }
